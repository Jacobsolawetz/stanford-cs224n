...
#         
#         else:
#             mask = [True] * sent_length
#             for i in range(sent_length, max_length):
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embedding_staging = tf.nn.embedding_lookup(embedding_tensor, self.input_placeholder, partition_strategy='mod', name='embed')
        embeddings = tf.reshape(embedding_staging,(-1,self.max_length,self.config.n_features*self.config.embed_size))
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            ...
            b2 = tf.get_variable("b", shape=[Config.n_classes], initializer=tf.constant_initializer())
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                output_drop_t  = tf.nn.dropout(output , dropout_rate)
        ...
        preds_stacked = tf.stack(preds)
        preds = tf.transpose(preds_stacked,perm=[1,0,2])
